...
def train_model(
    ...
    train_dataset = KeypointDataset(
        ...
        keypoints_dir=os.path.join(train_dir, 'keypoints'),  # 改为keypoints文件夹
        angles_dir=os.path.join(train_dir, 'centerline_angles'),  # 新增angles文件夹
        train=True,
    ...
    val_dataset = KeypointDataset(
        ...
        keypoints_dir=os.path.join(val_dir, 'keypoints'),  # 改为keypoints文件夹
        angles_dir=os.path.join(val_dir, 'centerline_angles'),  # 新增angles文件夹
        train=False
    )
    ...
    criterion = KeypointLoss(
        ...
        x_weight=0.6,  # x坐标的权重
        y_weight=0.3,  # y坐标的权重
        angle_weight=0.0995,  # 角度损失的权重
        constraint_weight=0.0005  # 约束损失的权重
    )
    ...
    for epoch in range(start_epoch, num_epochs):
            ...
            try:
                ...
                with autocast():
                    ...
                    loss, metrics = criterion(
                        ...
                        target_angles=batch['angles'].to(device, non_blocking=True),  # 需要从batch中获取角度标签
                        mask=None
                    )
                ...
                pbar.set_postfix({
                    ...
                    'x_loss': f'{metrics["x_loss"]:.6f}',
                    'y_loss': f'{metrics["y_loss"]:.6f}',
                    'angle_loss': f'{metrics["angle_loss"]:.6f}',
                    'constraint_loss': f'{metrics["constraint_loss"]:.6f}'
                })
            except Exception as e:
                ...
                if 'pred_angles' in locals(): del pred_angles  # 新增
            ...
            for batch in pbar:
                ...
                with autocast():
                    ...
                    loss, metrics = criterion(
                        ...
                        target_keypoints=keypoints,
                        pred_angles=pred_angles,
                        target_angles=batch['angles'].to(device, non_blocking=True),
                        mask=None
                    )
                ...
                # 清理显存
                del images, keypoints, pred_keypoints, pred_angles, loss
                clean_memory()
        ...
        print(f'Train Angle Error: {safe_mean(train_metrics["mean_angle_error"]):.6f}, '
              f'Val Angle Error: {safe_mean(val_metrics["mean_angle_error"]):.6f}')
        print(f'Train Constraint Loss: {safe_mean(train_metrics["constraint_loss"]):.6f}, '  # 新增
              f'Val Constraint Loss: {safe_mean(val_metrics["constraint_loss"]):.6f}')  # 新增
...
if __name__ == '__main__':
    ...
    # 设置训练参数
    train_params = {
        'train_dir': r"I:\RA-MED\VertTiltFormer\keypoint_detection\Data\dataset\train",
        'val_dir': r"I:\RA-MED\VertTiltFormer\keypoint_detection\Data\dataset\test",
